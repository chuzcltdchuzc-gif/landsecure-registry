...
import { MapPin, Upload, Save, AlertTriangle, Shield, Zap, Lock } from "lucide-react";
...
export default function EhimeRegisterLand() {
  ...
  const handleWardChange = (wardCode) => {
    ...
    setGeneratedParcelNumber(""); // reset when ward changes
  };

  const handlePropertyTypeChange = (pt) => {
    set("property_type", pt);
    setGeneratedParcelNumber(""); // reset when type changes
  };
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      await base44.entities.AuditLog.create({
        ...
        details: JSON.stringify({ parcel_number: generatedParcelNumber, ward: form.ward, property_type: form.property_type }),
      });
    },
    onSuccess: () => {
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <p className="text-xs text-muted-foreground">
            Draw the parcel boundary on the map. Area and perimeter are calculated automatically. Overlap with existing parcels is detected in real time.
          </p>
